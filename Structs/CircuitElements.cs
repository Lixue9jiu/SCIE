...
{
	[Serializable]
	public abstract class CircuitElement : Element
	{
		protected CircuitElement(ElementType type) : base(type)
		{
		}
		protected CircuitElement(SerializationInfo info, StreamingContext context) : base(info, context)
        {
			Next = (DynamicArray<CircuitElement>)info.GetValue("Next", typeof(DynamicArray<CircuitElement>));
        }
		...
		public static void GenerateWireVertices(BlockGeometryGenerator generator, int value, int x, int y, int z, int mountingFace, float centerBoxSize, Vector2 centerOffset, TerrainGeometrySubset subset)
		{
			...
			var device = SubsystemEnergy.GetCircuitElement(terrain.GetCellValueFast(x, y, z)) as CircuitDevice;
			if (device == null) return;
			Color color = WireBlock.WireColor;
			int num = Terrain.ExtractContents(value);
			if (num == EnergyElementBlock.Index)
			{
				int? color2 = EnergyElementBlock.GetColor(Terrain.ExtractData(value));
				if (color2.HasValue)
				...
				if ((num4 & (1 << tmpConnectionPath.ConnectorFace)) == 0)
				{
					...
					if (centerOffset != Vector2.Zero || connectorDirection != ElectricConnectorDirection.In)
					{
						...
						if (num != EnergyElementBlock.Index)
						{
							...
							if (Terrain.ExtractContents(cellValue) == EnergyElementBlock.Index)
							{
								int? color4 = EnergyElementBlock.GetColor(Terrain.ExtractData(cellValue));
								if (color4.HasValue)
								{
	...
	public abstract class Resistor : CircuitElement
	{
		protected Resistor() : base(ElementType.Resistor)
		{
		}
	}
	...
	public abstract class IC : CircuitElement, IEquatable<IC>
	{
