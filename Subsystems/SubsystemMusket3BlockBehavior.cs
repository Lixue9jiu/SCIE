...
namespace Game
{
	public class SubsystemMusket3BlockBehavior : SubsystemMusket2BlockBehavior
	{
							...
							case AimState.InProgress:
								{
									...
									if (m_subsystemTime.PeriodicGameTimeEvent(0.17, 0) && Musket3Block.GetBulletNum(Terrain.ExtractData(inventory.GetSlotValue(activeSlotIndex))) > 0)
									{
										...
										// else
										{
											...
											var vector31 = Vector3.Normalize(view5.ActiveCamera.ViewDirection);
		...
		{
			return value != Terrain.MakeBlockValue(521, 0, Bullet2Block.SetBulletType(0, Bullet2Block.BulletType.HandBullet)) || Musket3Block.GetBulletNum(Terrain.ExtractData(inventory.GetSlotValue(slotIndex))) > 62
				? 0
				: 1;
		}
