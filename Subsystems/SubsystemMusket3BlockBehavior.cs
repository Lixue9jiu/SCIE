				...
				if (activeSlotIndex >= 0)
				{
					...
					if (num == Musket3Block.Index && slotCount > 0)
					{
						...
						case AimState.InProgress:
						{
							...
							if (num4 > 0.2f && !Musket3Block.GetHammerState(Terrain.ExtractData(num2)))
							{
						...
						case AimState.Completed:
						{
							...
							Musket3Block.LoadState loadState = Musket3Block.GetLoadState(data);
							if (Musket3Block.GetHammerState(Terrain.ExtractData(num2)))
							{
								switch (loadState)
								{
								case Musket3Block.LoadState.bullet:
									...
									goto default;
								default:
									if (loadState == Musket3Block.LoadState.bullet2)
									{
										flag = true;
										value2 = Terrain.MakeBlockValue(214, 0, BulletBlock.SetBulletType(0, BulletBlock.BulletType.IronBullet));
										s = 320f;
							...
							if (flag)
							{
								...
								if (loadState == Musket3Block.LoadState.bullet)
									num2 = Terrain.MakeBlockValue(Terrain.ExtractContents(num2), 0, Musket3Block.SetLoadState(Terrain.ExtractData(num2), Musket3Block.LoadState.Empty));
								if (loadState == Musket3Block.LoadState.bullet2)
									num2 = Terrain.MakeBlockValue(Terrain.ExtractContents(num2), 0, Musket3Block.SetLoadState(Terrain.ExtractData(num2), Musket3Block.LoadState.bullet));
								num3 = 1;
							}
							if (Musket3Block.GetHammerState(Terrain.ExtractData(num2)))
							{
								num2 = Terrain.MakeBlockValue(Terrain.ExtractContents(num2), 0, Musket3Block.SetHammerState(Terrain.ExtractData(num2), false));
