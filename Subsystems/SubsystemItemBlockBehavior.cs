...
namespace Game
{
	...
	public class SubsystemItemBlockBehavior : SubsystemThrowableBlockBehavior, IDrawable
	{
		...
		public override bool OnAim(Vector3 start, Vector3 direction, ComponentMiner componentMiner, AimState state)
		{
			...
			return Terrain.ExtractContents(value) != GunpowderBlock.Index && (item is OreChunk || item is Mould && (value != ItemBlock.IdTable["Telescope"]) || item is Brick || value == ItemBlock.IdTable["ScrapIron"]) && base.OnAim(start, direction, componentMiner, state);
		}
