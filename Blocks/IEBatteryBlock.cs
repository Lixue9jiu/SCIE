...
namespace Game
{
	...
	public class IEBatteryBlock : FlatBlock, IDurability
	{
		...
		{
			m_standaloneBlockMesh.AppendMesh("Models/Battery", "Battery",Matrix.CreateScale(0.8f), Matrix.CreateTranslation(9f / 16f, -7f / 16f, 0f), Color.DarkGray);
			...
			m_standaloneBlockMesh3.AppendMesh("Models/Saw", "Saw", Matrix.CreateRotationX(1.6f) * Matrix.CreateRotationY(1.6f) * Matrix.CreateRotationY(0.8f) * Matrix.CreateRotationZ(0.8f) * Matrix.CreateTranslation(0.5f, -0.5f, 0f) * Matrix.CreateScale(.75f), Matrix.CreateTranslation(9f / 16f, -16f / 16f, 0f)*Matrix.CreateScale(100f), Color.White);
			...
			m_standaloneBlockMesh4.AppendMesh("Models/Battery", "Battery", Matrix.CreateRotationZ(-MathUtils.PI / 2) * Matrix.CreateScale(.7f) * Matrix.CreateTranslation(-0.2f, 0.3f, 0f), Matrix.CreateTranslation(9f / 16f, -7f / 16f, 0f), Color.DarkGray);
			m_standaloneBlockMesh4.AppendMesh("Models/Battery", "Battery", Matrix.CreateScale(.5f) * Matrix.CreateTranslation(0.1f, -0.3f, 0f), Matrix.CreateTranslation(9f / 16f, -7f / 16f, 0f), Color.DarkGray);
			base.Initialize();
		}
