...
namespace Game
{
	partial class ItemBlock
	{
		internal static void InitItems()
		{
			...
			new Mould("Models/Piston", "Piston", Matrix.CreateTranslation(0.5f, 0.3f, 0.5f) * 1.2f, Matrix.CreateTranslation(4f, 3.8f, 0f), "这个由铁，铜和钢制成的活塞，是许多机器的必要部件。", "IndustrialPiston", 1.6f),
			new Wire("CopperWire"),
			...
			new Mould("Models/Piston2", "Cylinder", Matrix.CreateTranslation(0.5f, 0.7f, 0.5f) * Matrix.CreateScale(0.6f), Matrix.CreateTranslation(4f, 3.8f, 0f), "这个由合金，铝和钢制成的圆筒，是许多机器的必要零件。", "Cylinder", 1.6f),
			...
			new ETrain(),
			new Mould("Models/Battery", "Battery", Matrix.CreateRotationX(MathUtils.PI / 2) * Matrix.CreateTranslation(0.5f, 0.5f, 0f), Matrix.CreateTranslation(9f / 16f, -7f / 16f, 0f) * Matrix.CreateScale(10f), "差速器", "差速器"),
